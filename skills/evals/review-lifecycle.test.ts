...
const spawn = (fork_turns: unknown = "none", agent_type = "oracle") => response({ type: "function_call", name: "spawn_agent", namespace: "collaboration", call_id: "call-1", arguments: JSON.stringify({ task_name: "review", agent_type, fork_turns, message: "Review the diff." }) })
...
describe("saved review lifecycle evidence", () => {
  test("pairs successful dispatch, independent result, parent final and completion", () => {
    expect(parse(...valid())).toEqual({ reviews: [{ taskName: "/root/review", role: "oracle", result: "No findings." }], failures: [] })
  ...
  test("rejects prose and command output claiming review", () => {
    rejected(start(), response({ type: "message", role: "assistant", content: [{ type: "output_text", text: "Oracle review passed." }] }), final(), complete())
  ...
  test("requires independent oracle role dispatch", () => {
