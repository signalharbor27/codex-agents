...
Boundaries: these are small synthetic projects, not general implementation, security, or research benchmarks. The setup oracle checks only its stated restart contract; it does not establish behavior for all malformed settings. Read-only scope cases test the initial decision frontier, not a complete interactive conversation. `review-assumption` tests reasoning about a supplied fix, not a real multistep review/fix cycle. Host skill discovery remains ambient. Repetition measures consistency; it does not remove prompt selection or grader bias.
