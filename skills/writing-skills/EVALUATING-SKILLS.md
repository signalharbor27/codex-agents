...
When comparing candidates, conceal version labels and expected conclusions from the judge, counterbalance execution order, and keep the scoring rubric outside the acting agent's context. Retain task requirements and realistic repository names, including tests. Preserve source hashes, model settings and label mappings in a separate provenance record. Inspect artifacts and tool records before judging a polished completion claim; include an incomplete result that the judge must reject. A small paired sample supports case-specific observations, not a general model-quality claim.
