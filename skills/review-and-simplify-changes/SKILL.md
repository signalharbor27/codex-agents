...
- Delegate substantive review to `reviewer`, using the built-in `review-agent` skill for defect review when available. Use `fast_reviewer` only for bounded mechanical evidence and `verifier` for command evidence. Include the selected role and its assigned concern in each brief.
