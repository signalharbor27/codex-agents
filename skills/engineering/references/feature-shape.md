...
- Use the `implementer` profile for agreed slices; it implements and runs focused proof. For substantial delegated slices, use fresh independent acceptance review grounded in the requirements and repository evidence. Select the review role through `review-and-simplify-changes`, including oracle escalation, and use a verifier for command evidence. Verification can overlap unrelated implementation when the tested files and dependencies are stable.
